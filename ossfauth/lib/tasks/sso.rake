#define sso of website console configuration tasks
require 'erb'

namespace 'sso' do
  desc 'SSO OpenFoundry Site-wide Configure'
  namespace 'config' do
    ...
    desc 'SSO OpenFoundry Module Configure'
    task 'module' do
    ...
    desc 'SSO OpenFoundry Environment Configure'
    task 'environment' do
    ...
    desc 'SSO OpenFoundry Translation Configure'
    task 'translation' do
  ...
  task :config => ['config:db', 'config:memcache', 'config:translation',
...
def get_bindings(binding_fname = nil)
  binding_fname||='sso.production.binding.rb'
